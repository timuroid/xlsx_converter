...
Статическое browser-only приложение для преобразования SAP XLSX-выгрузки: пользователь выбирает один или несколько файлов, приложение добавляет расчетные колонки `T:W` с Excel-формулами и отдает готовые XLSX обратно на скачивание.
